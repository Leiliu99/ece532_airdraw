...
//	menu_display(frame, option, curr_clr);
	...
	while (1) {
		...
		if (option == DRAW) {
			...
			if (is_ccc == 1) {
				int ccc_index = 0;
				int xloc_curr_ccc = 50;
				int yloc_curr_ccc = 50;
				while (1) {
				...
				backspace(frame, xloc_curr_ccc, yloc_curr_ccc, (xloc_curr_ccc - 50), 0);
				...
				//TODO: add counter for the toggling cursor YA!
					...
					if (ps2_data != *keyboard) {
						...
						if(((ps2_data & 0xff) != 0xf0) && ((ps2_data & 0xf000) == 0xf000)) {
							...
							if (text_index == 0) {
								if (curr_clr == PIXEL_USER) {
									value = ps2_display(frame, ps2_data, xloc_curr, yloc_curr, colour_ccc, text_index, 0, 1);
								} else {
									value = ps2_display(frame, ps2_data, xloc_curr, yloc_curr, curr_clr, text_index, 0, 1);
								}

							} else {
								if (curr_clr == PIXEL_USER) {

									value = ps2_display(frame, ps2_data, xloc_curr, yloc_curr, colour_ccc, text_index, chartype[text_index], 1);
								} else {
									value = ps2_display(frame, ps2_data, xloc_curr, yloc_curr, curr_clr, text_index, chartype[text_index], 1);
								}
							}
